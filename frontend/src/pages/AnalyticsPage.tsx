    ...
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        <StatCard 
          ...
          value={isLoadingSummary ? '...' : (summary?.total_consultations ?? 0).toLocaleString()} 
          sub="↑ 12.4% this month" 
          subColor="#0e7c4a" 
        />
        <StatCard 
          label="Avg. Documentation Time" 
          value={isLoadingSummary ? '...' : `${summary?.avg_duration_minutes ?? 14}m`} 
          sub="↓ 78% vs manual (14m)" 
          subColor="#0e7c4a" 
        />
        <StatCard 
          label="AI Accuracy Rate" 
          value={isLoadingSummary ? '...' : `${summary?.ai_accuracy_rate ?? 98}%`} 
          sub="↑ 0.3% vs last month" 
          subColor="#0e7c4a" 
        />
        <StatCard 
          label="Reports Exported" 
          value={isLoadingSummary ? '...' : (summary?.reports_exported ?? 0).toLocaleString()} 
