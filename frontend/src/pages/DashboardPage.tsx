  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        ...
        {/* Compliance / KPIs */}
        <div className="card">
          <div style={{ padding: '18px 22px 14px', borderBottom: '1px solid var(--border)' }}><h3 style={{ fontSize: 16 }}>System Performance KPIs</h3></div>
          <div style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            {isLoadingKpis ? (
              <div className="animate-pulse" style={{ height: 100, background: 'var(--surface-2)', borderRadius: 4 }}></div>
            ) : kpis.length === 0 ? (
              <div style={{ color: 'var(--text-3)' }}>No KPIs found</div>
            ) : kpis.map((k: any) => (
              <div key={k.metric}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <span style={{ fontSize: 13, fontWeight: 500 }}>{k.metric}</span>
                  <Badge variant={k.met ? 'green' : 'amber'}>{k.current}</Badge>
                </div>
                <div style={{ background: 'var(--surface-2)', borderRadius: 20, height: 8, overflow: 'hidden' }}>
                  <div style={{ height: '100%', borderRadius: 20, background: k.met ? '#0e7c4a' : 'var(--teal-mid)', width: k.metric.includes('Response') ? '80%' : '100%', transition: 'width 0.4s' }} />
                </div>
              </div>
            ))}
          </div>
